...
class BertDetector():
    ...
    def tokenize(self, x, should_log=True):
      input_ids = []
      attention_masks = []
      i = 0
      size = len(x)
      five_percent = size*0.05 
      if should_log: print('Starting to tokenize data\n')
      for tweet in x:
          proccessed_percante = i/size*100
          if i%five_percent == 0 and should_log:
      ...
      if should_log: print('Done Tokenizing\n')
    ...
    def tokenize_and_get_dataset_loader(self, x, y, sample_randomly=False, should_log=False):
        input_ids, attention_masks = self.tokenize(x, should_log)
    ...
    def get_prediction(self, x, y, should_log=False):
        dataset_loader = self.tokenize_and_get_dataset_loader(x, y, should_log)
    ...
    def train(self, x_train, y_train, x_val, y_val, epochs=4):
        ...
        for epoch in range(0, epochs):
            ...
            val_acc, val_loss, probabilites, predictions, _ = self.get_prediction_with_dloader(val_dataloader)
